...
export default function EditWorkingProcessPage() {
  ...
  useEffect(() => {
    const fetchWorkingProcess = async () => {
      ...
      try {
        ...
        if (error) {
          console.error("Error fetching working process:", error);
          toast.error("Failed to load working process data");
          router.push("/admin/working-processes");
          return;
        }

        if (!data) {
          toast.error("Working process not found");
          router.push("/admin/working-processes");
          return;
        }
  ...
  const onSubmit = async (data: WorkingProcessFormData) => {
    try {
      ...
      toast.success("Working process updated successfully!");
      router.push("/admin/working-processes");
    } catch (error) {
      console.error("Error updating working process:", error);
      toast.error("Failed to update working process. Please try again.");
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        toast.error("Failed to delete working process. Please try again.");
        ...
        <div className="mb-6">
          ...
          {workingProcess && (
            <p className="mt-1 text-sm text-gray-500">
              Editing: Step {workingProcess.step_no} - {workingProcess.title}
            </p>
          )}
        </div>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete &quot;{workingProcess?.title}
