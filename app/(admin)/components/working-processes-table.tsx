...
export function WorkingProcessesTable({
  ...
  // Calculate stats
  const totalWorkingProcesses = workingProcesses.length;
  const activeWorkingProcesses = workingProcesses.filter(
    (s) => s.is_active
  ).length;
  const inactiveWorkingProcesses = workingProcesses.filter(
    (s) => !s.is_active
  ).length;

  // Handle delete
  const handleDelete = (workingProcess: WorkingProcess) => {
    setDeleteTarget(workingProcess);
      ...
      <div className="flex flex-col gap-4 mb-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">
            Manage your working process steps
          </p>
        </div>
        <Button asChild className="bg-black">
          <Link href="/admin/working-processes/new">
        ...
        <Table>
          ...
          <TableBody>
            {workingProcesses.map((process) => (
              ...
              >
                <TableCell>
                  <span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-gray-100 text-sm font-semibold text-gray-900">
                    {process.step_no}
                  </span>
                </TableCell>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-32">
                      <DropdownMenuItem asChild>
                        <Link
                          href={`/admin/working-processes/${process.id}/edit`}
                        >
                          Edit
                        </Link>
                      </DropdownMenuItem>
        ...
        {workingProcesses.length === 0 && (
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Get started by creating a new process step.
            </p>
